...
CBUFFER_START(_Terrain)
    ...
    // Добавлены параметры для смешивания по высоте
    half _HeightScale0, _HeightScale1, _HeightScale2, _HeightScale3;
    half _HeightOffset0, _HeightOffset1, _HeightOffset2, _HeightOffset3;
...
// Текстуры высот для каждого слоя
...
// Функция для чтения высот слоев
half4 SampleHeights(float4 uvSplat01, float4 uvSplat23)
{
    half4 heights = half4(0.5h, 0.5h, 0.5h, 0.5h);
    
#ifdef _TERRAIN_BLEND_HEIGHT
    heights.x = SAMPLE_TEXTURE2D(_Height0, sampler_Splat0, uvSplat01.xy).r;
    heights.y = SAMPLE_TEXTURE2D(_Height1, sampler_Splat0, uvSplat01.zw).r;
    heights.z = SAMPLE_TEXTURE2D(_Height2, sampler_Splat0, uvSplat23.xy).r;
    heights.w = SAMPLE_TEXTURE2D(_Height3, sampler_Splat0, uvSplat23.zw).r;
    
    // Применяем масштаб и смещение высот
    heights.x = saturate(heights.x * _HeightScale0 + _HeightOffset0);
    heights.y = saturate(heights.y * _HeightScale1 + _HeightOffset1);
    heights.z = saturate(heights.z * _HeightScale2 + _HeightOffset2);
    heights.w = saturate(heights.w * _HeightScale3 + _HeightOffset3);
#endif
