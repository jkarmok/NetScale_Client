...
void HeightBasedSplatModify(inout half4 splatControl, in half4 heights)
{
    // Применяем высоты к весам слоев
    half4 heightWeights = splatControl * heights;
    
    // Находим максимальную высоту
    half maxHeight = max(max(heightWeights.r, heightWeights.g), max(heightWeights.b, heightWeights.a));
    
    // Используем параметр перехода для плавности смешивания
    half transition = _HeightTransition;
    
    // Вычисляем новые веса на основе высот
    // Вычитаем из каждой высоты максимальную высоту минус transition
    // Это даст положительные значения только для слоев с высотой близкой к максимальной
    half4 weightedHeights = heightWeights + transition - maxHeight.xxxx;
...
#ifdef _TERRAIN_BLEND_HEIGHT
    // Читаем высоты слоев
    half4 heights = SampleHeights(IN.uvSplat01, IN.uvSplat23);
    
    // Применяем смешивание по высоте только если есть не более 4 слоев
    // (многопроходный рендеринг нарушает нормализацию)
    if (_NumLayersCount <= 4)
