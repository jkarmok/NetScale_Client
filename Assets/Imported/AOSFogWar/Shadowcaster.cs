...
namespace FischlWorks_FogWar
{
    ...
    public class Shadowcaster
    {
        ...
        public class FogField
        {
            ...
            {
                // Оптимизация: используем Parallel.For для больших уровней
                if (levelRow.Count * levelRow[0].Count() > 1000)
                {
                    System.Threading.Tasks.Parallel.ForEach(levelRow, column =>
                    {
                        column.Reset(fogWar);
                    });
                }
            ...
            public Color[] GetColors(float fogPlaneAlpha, csFogWar fogWar)
            {
                ...
                int totalTiles = levelRow.Count * levelRow[0].Count();
                
                // Оптимизация: используем Parallel.For для больших уровней
                if (totalTiles > 1000)
                {
                    System.Threading.Tasks.Parallel.For(0, levelRow[0].Count(), xIterator =>
                    {
                        for (int yIterator = 0; yIterator < levelRow.Count; yIterator++)
                        {
                            SetColorAtIndex(xIterator, yIterator, fogPlaneAlpha, fogWar);
                        }
                    });
                }
                else
            ...
            private void SetColorAtIndex(int xIterator, int yIterator, float fogPlaneAlpha, csFogWar fogWar)
            {
        ...
        public class LevelColumn
        {
            ...
            public void Reset(csFogWar fogWar)
            {
                // Оптимизация: используем простой цикл вместо LINQ
                int count = levelColumn.Count;
                bool keepRevealed = fogWar.keepRevealedTiles;
        ...
        private class ColumnIterator
        {
            ...
            public List<Vector2Int> GetTiles()
            {
                ...
                quadrantPoints.Capacity = maxRow - minRow + 1;
        ...
        // Класс для batch-обработки revealer'ов
        private class BatchRevealerData
        {
            public Vector2Int position;
            public int sightRange;
            
            public BatchRevealerData(Vector2Int pos, int range)
            {
                position = pos;
                sightRange = range;
            }
        ...
        private Dictionary<Vector2Int, bool> tileObstacleCache = new Dictionary<Vector2Int, bool>();
        ...
        public void Initialize(csFogWar fogWar)
        {
            ...
            // Предварительное кэширование препятствий для быстрого доступа
            CacheObstacles();
        }
        
        private void CacheObstacles()
        {
            tileObstacleCache.Clear();
            int xDim = fogWar.levelData.levelDimensionX;
            int yDim = fogWar.levelData.levelDimensionY;
            ...
            for (int x = 0; x < xDim; x++)
            {
                for (int y = 0; y < yDim; y++)
                {
                    ...
                    tileObstacleCache[pos] = fogWar.levelData[x][y] == csFogWar.LevelColumn.ETileState.Obstacle;
                }
            }
        }
        ...
        {
            ProcessRevealerInternal(revealerPoint, sightRange);
        }
        
        /// Batch processing for multiple revealers
        public void ProcessLevelDataBatch(Vector2Int revealerPoint, int sightRange, int totalRevealers)
        {
            ProcessRevealerInternal(revealerPoint, sightRange);
        }
        
        private void ProcessRevealerInternal(Vector2Int revealerPoint, int sightRange)
        {
            // Reveal the first tile where the revealer is at
            Vector2Int levelPos = fogWar.WorldToLevel(fogWar.GetWorldVector(revealerPoint));
            if (fogWar.CheckLevelGridRange(levelPos))
            {
                fogField[levelPos.x][levelPos.y] = LevelColumn.ETileVisibility.Revealed;
            }
        ...
        private void RevealTileIteratively(Vector2Int quadrantPoint, int sightRange)
        {
            ...
            if (!fogWar.CheckLevelGridRange(levelCoordinates) || quadrantPoint.magnitude > sightRange)
            {
                return;
            }
        ...
        private bool IsTileEmpty(Vector2Int quadrantPoint)
        {
            ...
            if (!fogWar.CheckLevelGridRange(levelCoordinates))
            {
                return true;
            }
            
            // Используем кэш для быстрого доступа
            if (tileObstacleCache.TryGetValue(levelCoordinates, out bool isObstacle))
            {
                return !isObstacle;
            }
